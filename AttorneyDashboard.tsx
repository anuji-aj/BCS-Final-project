...
import { 
  Gavel, Search, X, User, FileText, Scale, 
  AlertCircle, File, Lock, Eye, Calendar, 
  CheckCircle 
} from 'lucide-react';
...
export function AttorneyDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  // --- STATE MANAGEMENT ---
  // State for handling search inputs and results
  ...
  // State for profile and password management
  const [showProfile, setShowProfile] = useState(false);
  const [newPass, setNewPass] = useState('');
  const [confirmPass, setConfirmPass] = useState('');
  
  // State for UI Notifications (Toast messages)
   ...
   * Auto-hides after 3 seconds to improve UX.
   */
  ...
  /**
   * Handles the search logic for retrieving case files.
   * Connects to the local simulated database.
   */
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (!term) {
        showNotification("Please enter a valid Case ID.", "error");
        return;
    }
    ...
    // Find the specific case matching the ID (case-insensitive)
    const match = allData.find((c: any) => c.id.toLowerCase() === term.toLowerCase());

    if (match) {
      setFoundCase(match);
      showNotification("Case record retrieved successfully.", "success");
    } else {
      // Handle "Not Found" scenario
      showNotification("Error: Case ID not found in the database.", "error");
  ...
  /**
   * Extracts Judicial/Court data from the record.
   * Handles inconsistencies in data structure (flat vs nested objects).
   */
  const getJudicialInfo = () => {
    if (!foundCase) return null;

    // Check for nested container objects first
    ...
    // Check if any relevant data exists to determine render logic
  ...
  /**
   * Extracts Medical/JMO data from the record.
   * Normalizes different naming conventions (jmoReport vs medicalReport).
   */
  ...
  const handleUpdatePassword = () => {
    // Regex enforces: Min 8 chars, 1 Uppercase, 1 Lowercase, 1 Number, 1 Symbol
    const regex = /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#\$%\^&\*])(?=.{8,})/;
    
    if (!regex.test(newPass)) {
        showNotification("Password must contain: 8+ chars, Upper, Lower, Number, Symbol.", "error");
        return;
    }
    ...
    // Simulate successful update and logout
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans flex flex-col relative">
      ...
      {/* NOTIFICATION COMPONENT
          Renders a floating toast message at the top of the screen
          based on the 'notification' state.
      */}
      {notification.show && (
        <div className={`fixed top-5 left-1/2 transform -translate-x-1/2 z-[300] flex items-center gap-3 px-6 py-4 rounded-xl shadow-2xl border-2 animate-bounce
      ...
      <main className="flex-1 p-6 max-w-5xl mx-auto w-full flex flex-col">
        ...
        {/* CONDITIONAL RENDERING: Search View vs. Details View */}
        {!foundCase ? (
          /* SEARCH VIEW */
          ...
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              {/* COLUMN 1: CASE META & JUDICIAL INFO */}
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-4xl h-[85vh] rounded-2xl overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="flex-1 bg-slate-200 p-4 overflow-y-auto flex justify-center items-center">
               ...
               {/* RENDER LOGIC: Display Image or PDF based on file type */}
               {previewDoc.fileData ? (
      ...
      {showProfile && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 z-[200]">
           <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl relative animate-in zoom-in-95">
              <button onClick={() => setShowProfile(false)} className="absolute top-4 right-4 text-slate-400 hover:text-red-500"><X/></button>
              
              <div className="mb-6">
                 <h2 className="font-bold text-lg text-slate-800">Security Settings</h2>
